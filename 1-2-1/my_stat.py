...
def np_to_latex(data, centering="c"):
...
class LeastSquares:
    ...
    def add_to_axes(self, ax, start = None, end = None, param_dict = {'linewidth': 1, 'linestyle': '--'}, label = ""):
        if start == None:
            start = np.min(self.xs)
        if end == None:
            end = np.max(self.xs)
        out = ax.plot(np.array([start, end]), np.array([self.k * start + self.b, self.k * end + self.b]), label=label, **param_dict)
        return out
